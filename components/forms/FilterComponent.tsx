...
import { Button } from "@/components/ui/button";
import { Patient } from "@/types/appwrite.types"; // Import the updated Patient type

type FilterComponentProps = {
  data: Patient[];
  setFilteredData: React.Dispatch<React.SetStateAction<Patient[]>>;
};
  ...
  const applyFilters = () => {
    const filtered = data.filter((item) => {
      ...
      const matchName =
        filters.name === "" ||
        item.name.toLowerCase().includes(filters.name.toLowerCase());
      return matchGender && matchState && matchCity && matchName;
    });
    setFilteredData(filtered);
  };
